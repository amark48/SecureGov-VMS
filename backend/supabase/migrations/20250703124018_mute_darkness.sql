/*
  # Rename badge_number to employee_number and Add Auto-Generation

  1. New Columns
    - Add `tenant_prefix` and `next_employee_sequence` columns to the `tenants` table
    - These will be used for automatic employee number generation

  2. Changes
    - Rename `badge_number` to `employee_number` in the `profiles` table
    - Update unique constraint to be tenant-specific
...
DROP INDEX IF EXISTS idx_profiles_badge_number;
